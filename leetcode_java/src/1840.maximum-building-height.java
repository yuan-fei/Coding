 ...
 * 
 */
/**https://leetcode.com/problems/maximum-building-height/discuss/1175064/Python3.-O(N*log(N)).-Using-stack.-Explanation-added.*/
// @lc code=start
class Solution {
    public int maxBuilding(int n, int[][] restrictions) {
        Arrays.sort(restrictions, (a, b) -> Integer.compare(a[0], b[0]));
        System.out.println(restrictions.length);
        Stack<int[]> s = new Stack<>();
        s.push(new int[]{1, 0});
        for(int[] r : restrictions){
        	while(!s.isEmpty() && r[0] - s.peek()[0] < s.peek()[1] - r[1]){
        		s.pop();
        	}
        	if(s.isEmpty() || r[0] - s.peek()[0] >= r[1] - s.peek()[1]){
        		s.push(r);
        	}
        }
        ...
        int[] cur = s.pop();
        int max = n - cur[0] + cur[1];
        while(!s.isEmpty()){
        	int[] pre = s.peek();
        	// max - H[l] + max - H[r] + 1 = r - l + 1 => (r - l + H[l] + H[r]) / 2
        	max = Math.max(max, (cur[0] - pre[0] + cur[1] + pre[1]) / 2);
        	cur = s.pop();
        }
        return max;
    }
