...
class Solution {
    public int cherryPickup(int[][] grid) {
        ...
        for(int[] r : grid){
        	...
        	for(int x = 0; x < Math.min(steps, m); x++){
        		for(int y = m - 1; y >= Math.max(m - steps, 0); y--){
        			int gain = r[x] + r[y] - (x == y ? r[x] : 0);
        			for(int i : new int[]{-1, 0 ,1}){
        				for(int j : new int[]{-1, 0 ,1}){
        					int prevX = x + i;
        					int prevY = y + j;
        					if(0 <= prevX && prevX < m && 0 <= prevY && prevY < m && dp[prevX][prevY] >= 0){
        						newDp[x][y] = Math.max(dp[prevX][prevY] + gain, newDp[x][y]);
        					}
        				}
        			}
        			max = Math.max(max, newDp[x][y]);
        		}
        	}
        	steps++;
        	dp = newDp;
