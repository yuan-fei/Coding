 ...
 * dollars.
 * - Drive passenger 2 from point 10 to point 12 for a profit of 12 - 10 + 3 =
 * 5 dollars.
 * - Drive passenger 5 from point 13 to point 18 for a profit of 18 - 13 + 1 =
 * 6 dollars.
...
class Solution {
    public long maxTaxiEarnings(int n, int[][] rides) {
        ...
        List<Integer>[] events = new List[100005];
        for(int i = 0; i < events.length; i++){
            events[i] = new ArrayList<>();
        }
        for(int i = 0; i < rides.length; i++){
            events[rides[i][1]].add(i);
        }
        ...
        for(int i = 0; i < dp.length; i++){
            ...
            for(int id : events[i]){
                dp[i] = Math.max(dp[i], dp[rides[id][0]] + rides[id][1] - rides[id][0] + rides[id][2]);
            }
        }
