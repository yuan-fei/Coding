 ...
 * 
 * 
 * However, the same arr is not K-increasing for k = 1 (because arr[0] >
 * arr[1]) or k = 3 (because arr[0] > arr[3]).
 * 
 * 
 ...
 * It is suboptimal to change the array to, for example, [6,7,8,9,10] because
 * it would take 5 operations.
 ...
 * i <= 5.
 * One of the ways we can make the array K-increasing is by changing arr[3] to
 * 4 and arr[5] to 5.
 * The array will now be [4,1,5,4,6,5].
 * Note that there can be other ways to make the array K-increasing, but none
 * of them require less than 2 operations.
 * 
 * 
 * 
 * Constraints:
...
class Solution {
    public int kIncreasing(int[] arr, int k) {
        int cnt = 0;
        for(int i = 0; i < k; i++){
            cnt += longestNondecreasingSequence(arr, i, k);
        }
        return cnt;
    }

    int longestNondecreasingSequence(int[] arr, int start, int gap){
        TreeSet<int[]> ts = new TreeSet<>((a, b) -> Integer.compare(a[0], b[0]) == 0 ? Integer.compare(a[1], b[1]) : Integer.compare(a[0], b[0]));
        int cnt = 0;
        for(int i = start; i < arr.length; i += gap){
            cnt++;
            int[] k = {arr[i], i};
            int[] hk = ts.higher(k);
            if(hk == null){
