 ...
 * $500 banknote, then the withdraw request will be rejected because the
 * machine will first try to use the $500 banknote and then be unable to use
 * banknotes to complete the remaining $100. Note that the machine is not
 * allowed to use the $200 banknotes instead of the $500 banknote.
 * 
 * 
 ...
 * [0,1,0,3,1].
 * atm.withdraw(600);        // Returns [-1]. The machine will try to use a
 * $500 banknote
...
class ATM {
    ...
    public int[] withdraw(int amount) {
        int[] diff = new int[5];
        for(int i = 4 ; i >= 0; i--){
            diff[i] = amount / denominations[i];
            diff[i] = (int)Math.min((long)diff[i], cnt[i]);
            amount -= diff[i] * denominations[i];
        }
        if(amount == 0){
            for(int i = 4 ; i >= 0; i--){
                cnt[i] -= diff[i];
