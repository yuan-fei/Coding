 ...
 * 
 * 
 * Even-indexed rows (0, 2, 4, ...) are cyclically shifted to the
 * left.
 * 
 * 
 * 
...
class Solution {
    public boolean areSimilar(int[][] mat, int k) {
        ...
        boolean ans = true;
        for(int i = 0; i < n; i++){
            ans &= areEquals(mat[i], (i % 2 == 0)? -k : k);
        }
        return ans;
    }

    boolean areEquals(int[] arr, int k){
        for(int i = 0; i < arr.length; i++){
            if(arr[i] != arr[Math.floorMod(i + k, arr.length)]){
                return false;
            }
        }
        return true;
