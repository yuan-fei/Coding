...
class Solution {
    ...
    void countOneLinearComponent(long[] cnt, int n){
        for(int d = 1; d < n; d++){
            cnt[d - 1] += 2 * (n - d);
        }
    }

    void countOneCircularComponent(long[] cnt, int n){
        for(int d = 1; d <= n / 2; d++){
            cnt[d - 1] += n;
            if(d * 2 < n){
                cnt[d - 1] += n;
    ...
    void countTwoDisjointLinearComponents(long[] cnt, int a, int b, int delta){
        for (int i = 1; i < a + b && i + delta - 1 < cnt.length; i++) {
           cnt[i + delta - 1] += 2 * Math.max(0, Math.min(i, a) - (i + 1 - Math.min(i, b)) + 1);
        }
    }

    void countPointToLinear(long[] cnt, int a, int delta){
        for(int i = 1; i <= a && i + delta - 1 < cnt.length; i++){
            cnt[i + delta - 1] += 2;
        }
    }

    void countLinearToCircular(long[] cnt, int a, int b){
        int half = (b - 1) / 2;
