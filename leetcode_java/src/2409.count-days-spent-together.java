...
class Solution {
    public int countDaysTogether(String arriveAlice, String leaveAlice, String arriveBob, String leaveBob) {
        int lA = toInterval(arriveAlice);
        int rA = toInterval(leaveAlice);
        int lB = toInterval(arriveBob);
        int rB = toInterval(leaveBob);
        return Math.max(0, Math.min(rA, rB) - Math.max(lA, lB) + 1);
    }

    int toInterval(String dt){
