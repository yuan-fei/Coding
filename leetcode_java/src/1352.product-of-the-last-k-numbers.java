...
class ProductOfNumbers {
	...
	List<Integer> suffixProd;
    public ProductOfNumbers() {
        suffixProd = new ArrayList<>();
        suffixProd.add(1);
    }
    ...
    public void add(int num) {
    	if(num == 0){
        	...
        	suffixProd.add(suffixProd.get(suffixProd.size() - 1) * num);
        }
    }
    
    public int getProduct(int k) {
        if(k + 1 > suffixProd.size()){
        	return 0;
        }
        else{
        	return suffixProd.get(suffixProd.size() - 1) / suffixProd.get(suffixProd.size() - k - 1);
        }
    }
}
