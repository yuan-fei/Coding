...
class Solution {
    public int[] nodesBetweenCriticalPoints(ListNode head) {
        ...
        while(cur != null && cur.next != null){
            if(pre < cur.val && cur.val > cur.next.val){
                criticalPoint.add(i);
            }
            else if(pre > cur.val && cur.val < cur.next.val){
                criticalPoint.add(i);
            }
            pre = cur.val;
            cur = cur.next;
        ...
        if(criticalPoint.size() > 1){
            res[1] = criticalPoint.get(criticalPoint.size() - 1) - criticalPoint.get(0);
            for(i = 0; i < criticalPoint.size(); i++){
                if(i > 0){
                    res[0] = Math.min(res[0], criticalPoint.get(i) - criticalPoint.get(i - 1));
                }
            }    
        }
        else{
