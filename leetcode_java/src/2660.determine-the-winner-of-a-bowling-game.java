 ...
 * The score of player1 equals to the score of player2, so, there is a draw,
 * and the answer is 0.
 * 
 * 
 * 
...
class Solution {
    public int isWinner(int[] player1, int[] player2) {
        int a = getScore(player1);
        int b = getScore(player2);
        if(a > b){
            return 1;
        }
        else if(b > a){
    ...
    int getScore(int[] arr){
        ...
        for(int i = 0; i < arr.length; i++){
            int multiplier = 1;
            if((i - 2 >= 0 && arr[i - 2] == 10) || (i - 1 >= 0) && arr[i - 1] == 10){
                multiplier = 2;
            }
            ret += multiplier * arr[i];
        }
        return ret;
    }
}
