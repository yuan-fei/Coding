...
class Solution {
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        TreeMap<int[], List<Integer>> tm = new TreeMap<>(Comparator.comparing((int[] x) -> x[1]).thenComparing(x -> x[0]));
        ...
        while(!q.isEmpty()){
            for(int x = q.size(); x > 0; x--){
                ...
                int[] curPos = cur.getValue();
                tm.putIfAbsent(curPos, new ArrayList<>());
                tm.get(curPos).add(curNode.val);
                if(curNode.left != null){
                    q.offer(new Pair<>(curNode.left, new int[]{curPos[0] + 1, curPos[1] - 1}));
                }
                if(curNode.right != null){
                    q.offer(new Pair<>(curNode.right, new int[]{curPos[0] + 1, curPos[1] + 1}));
                }
            }
        }
