 ...
 * value of |a[i] - a[j]|, where 0 <= i < j < a.length and a[i] != a[j]. If all
...
class Solution {
    public int[] minDifference(int[] nums, int[][] queries) {
        ...
        for(int i = 0; i < queries.length; i++){
        	ret[i] = getMinDiff(queries[i][0], queries[i][1], pos);
        }
        return ret;
    }

    int getMinDiff(int l, int r, List<Integer>[] pos){
    	int MAX = 105;
    	int last = -1;
    	int min = MAX;
    	for(int x = 1; x <= 100; x++){
    ...
    boolean exist(int lb, int rb, List<Integer> l){
    	lb--;
    	int indexL = Collections.binarySearch(l, lb);
    	indexL = indexL < 0 ? (-indexL - 2) : indexL;
    	int indexR = Collections.binarySearch(l, rb);
    	indexR = indexR < 0 ? (-indexR - 2) : indexR;
    	// System.out.println(lb + ", " + rb + ", " + l + indexL + ", " + indexR);
