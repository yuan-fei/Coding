 ...
 * such that the i^th subarray is equal to groups[i] (0-indexed), and if i > 0,
 * the (i-1)^th subarray appears before the i^th subarray in nums (i.e. the
...
class Solution {
    public boolean canChoose(int[][] groups, int[] nums) {
        ...
        for(int i = 0; i < n; i++){
        	boolean found = false;
        	while(!found && cur < nums.length){
        		found = true;
        		for(int j = 0; j < groups[i].length; j++){
        			if(cur + j >= nums.length || nums[cur + j] != groups[i][j]){
        				found = false;
        				break;
        			}
        		}
