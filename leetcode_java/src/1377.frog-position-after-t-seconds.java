...
class Solution {
    ...
    public double frogPosition(int n, int[][] edges, int t, int target) {
        ...
        return dfs(-1, 1, 1, t, target);
    }

    double dfs(int p, int u, double prob, int t, int target){
    	...
    	int children = adj[u].size() - ((p == -1) ? 0 : 1);
    	if(children == 0){
    		if(u == target){
    			return prob;
    		}
    	...
    	for (int c : adj[u]) {
    		if(c != p){
	    		double ret = dfs(u, c, prob / children, t - 1, target);
	    		if(ret > 0){
	    			return ret;
	    		}	
    		}
