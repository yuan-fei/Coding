...
class Solution {
    public int minAbsoluteSumDiff(int[] nums1, int[] nums2) {
        ...
        for(int i = 0; i < nums1.length; i++){
        	...
        	sum += v;
        	sum %= MOD;
        	Integer ceiling = ts.ceiling(nums2[i]);
        	Integer floor = ts.floor(nums2[i]);
        	if(ceiling != null){
        		maxDiff = Math.max(maxDiff, v - Math.abs(nums2[i] - ceiling));
        	}
        	if(floor != null){
        		maxDiff = Math.max(maxDiff, v - Math.abs(nums2[i] - floor));
        	}
        }
        return (MOD + sum - maxDiff) % MOD;
    }
}
// @lc code=end
