 ...
 * and apply the discount if needed. Answers within 10^-5 of the actual value
...
class Cashier {
	...
	int n; 
	double rate; 
	Map<Integer, Integer> products2prices;
	int counter = 0;
    public Cashier(int n, int discount, int[] products, int[] prices) {
        this.n = n;
        this.rate = 1 - (double)discount / 100;
        products2prices = new HashMap<>();
        for(int i = 0; i < products.length; i++){
        	products2prices.put(products[i], prices[i]);
        }
    }
    ...
    public double getBill(int[] product, int[] amount) {
        ...
        for(int i = 0; i < product.length; i++){
        	total += products2prices.get(product[i]) * amount[i];
        }
        counter++;
        if(counter % n == 0){
        	return rate * total;
