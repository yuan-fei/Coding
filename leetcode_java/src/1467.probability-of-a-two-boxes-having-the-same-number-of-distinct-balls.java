 ...
 * 2,1], [2,1 / 1,3], [2,1 / 3,1], [2,3 / 1,1], [3,1 / 1,2], [3,1 / 2,1], [3,2
 * / 1,1]
...
class Solution {
    public double getProbability(int[] balls) {
    	...
    	int ub = total / 2;
    	double[][][] dp = new double[ub + 1][1 << n][1 << n];
    	dp[0][0][0] = 1;
        for (int i = 0; i < balls.length; i++) {
        	for(int j = 1; j <= balls[i]; j++){
        		double[][][] newDp = new double[ub + 1][1 << n][1 << n];
        		for (int k = 0; k <= ub; k++) {
        			for(int fi = 0; fi < (1 << (i + 1)); fi++){
        				for(int se = 0; se < (1 << (i + 1)); se++){
        					if(k < ub){
        						newDp[k + 1][fi|(1 << i)][se] += dp[k][fi][se] * 0.5;	
        					}
        					newDp[k][fi][se|(1 << i)] += dp[k][fi][se] * 0.5;
        				}
        			}
        		}
        		dp = newDp;
        ...
        double probEqual = 0;
        for(int fi = 0; fi < (1 << n); fi++){
			for(int se = 0; se < (1 << n); se++){
				if(Integer.bitCount(fi) == Integer.bitCount(se)){
					probExpected += dp[ub][fi][se];
				}
				probEqual += dp[ub][fi][se];
			}
		}
		return probExpected / probEqual;
    }
}
// @lc code=end
