...
class Solution {
    public int minCost(int[] houses, int[][] cost, int m, int n, int target) {
        ...
        // int[][] costMap = getCostMap(houses, cost, m, n);
        	...
        	for(int t = 1; t <= target; t++){
    			for (int c1 = 1; c1 <= n; c1++) {
    				...
    				for (int c2 = 1; c2 <= n; c2++) {
    					int cstTotal = cst + (c1 == c2 ? dp[i - 1][t][c2] : dp[i - 1][t - 1][c2]);
    					dp[i][t][c1] = Math.min(dp[i][t][c1], cstTotal);
    				}
    			}
        	}
        }
        ...
        int min = MAX;
        for (int c = 1; c <= n; c++) {
        	min = Math.min(dp[m][target][c], min);
        }
