...
class Solution {
    public TreeNode reverseOddLevels(TreeNode root) {
        ...
        layers.get(layers.size() - 1).add(root);
        ...
        while(!q.isEmpty()){
            ...
            for(int x = q.size(); x > 0; x--){
                TreeNode cur = q.poll();
                if(cur.left != null){
                    layers.get(layers.size() - 1).add(cur.left);
                    layers.get(layers.size() - 1).add(cur.right);  
                    q.offer(cur.left);
                    q.offer(cur.right);
                }
            }
            l++;
            if(l % 2 == 1){
                Collections.reverse(layers.get(layers.size() - 1));
            }
        }
        ...
        for(int i = 0; i < layers.size() - 1; i++){
            for(int j = 0; j < layers.get(i).size(); j++){
                layers.get(i).get(j).left = layers.get(i + 1).get(j * 2);
                layers.get(i).get(j).right = layers.get(i + 1).get(j * 2 + 1);
            }
        }
        return root;
    }
