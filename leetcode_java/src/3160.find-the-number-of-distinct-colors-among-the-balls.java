 ...
 * need to find the number of colors among the balls.
 * 
 * Return an array result of length n, where result[i] denotes the number of
 * colors after i^th query.
 * 
...
class Solution {
    public int[] queryResults(int limit, int[][] queries) {
        ...
        // for query i: [ball, color], find color of the ball through ballToColor,
        // update colorToCount
        ...
        for (int i = 0; i < ans.length; i++) {
            ...
            if (ballToColor.containsKey(ball)) {
                ...
                colorToCount.put(oldColor, colorToCount.get(oldColor) - 1);
                colorToCount.remove(oldColor, 0);
            }
