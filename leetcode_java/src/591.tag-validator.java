...
class Solution {
    ...
    public boolean isValid(String code) {
        s = code;
        return checkClosedTag() && idx + 1 == code.length();
    }
    ...
    boolean checkClosedTag(){
    	if(idx >= s.length()){
    		return false;
    	}
    	int start = idx;
    	if(checkStartTag()){
    		String tagName = s.substring(start + 1, idx);
    		d("After checkStartTag: " + tagName);
    		idx++;
    		skipTagContent();
    		d("After skipTagContent: " + idx);
			start = idx;
			if(checkEndTag()){
				return tagName.equals(s.substring(start + 2, idx));
			}
    	}
    	return false;
    }
    
    boolean checkStartTag(){
    	if(idx >= s.length()){
    		return false;
    	}
    	if(s.charAt(idx) == '<'){
    ...
    void skipTagContent(){
    	int start = idx;
    	skipNonTagStart();
    	d("After skipNonTagStart: " + idx);
    	while(idx < s.length() && (checkCData() || checkClosedTag())){
    		idx++;
    		skipNonTagStart();
    		d("After skipNonTagStart: " + idx);
    	}
    ...
    boolean checkCData(){
    	...
    	if(checkCDataStart()){
    		idx++;
    		skipCDataContent();
    		d("After skipCDataContent: " + idx);
    		idx++;
    		return checkCDataEnd();
    	}
    	idx = start;
    	return false;
    }

    boolean checkCDataStart(){
    	// d("begin checkCDataStart: " + idx);
    	if(s.startsWith("<![CDATA[", idx)){
    		idx += "<![CDATA[".length() - 1;
    		return true;
    	}
    	return false;
    ...
    void skipCDataContent(){
    	while(idx < s.length() && !s.startsWith("]]>", idx)){
    		idx++;
    	}
    	if(idx < s.length()){
    		idx--;
    ...
    boolean checkTagName(){
    	...
    	int start = idx;
    	for(int i = 1; i <= 9; i++){
    		if(idx < s.length() && Character.isUpperCase(s.charAt(idx))){
    			idx++;
    		}
    		else{
    			break;
