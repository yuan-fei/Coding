 ...
 * 
 * Note: The answer should be the minimum product before the modulo operation
 * is done.
 * 
 * 
 * Example 1:
 ...
 * - In the first operation we can swap the leftmost bit of the second and
 * fifth elements.
...
class Solution {
    public int minNonZeroProduct(int p) {
        ...
        long base = max - 1;
        long exp = (1L << (p - 1)) - 1;
        // System.out.println(Arrays.asList(max, base, exp, pow(base, exp)));
        return (int)((max * pow(base, exp)) % MOD);
    }
    int MOD = (int)1e9 + 7;
    long pow(long base, long exp){
        ...
        if(exp % 2 == 0){
            long t = pow(base, exp / 2);
            return (t * t) % MOD;
        }
        else{
            long t = pow(base, exp - 1);
            return (base * t) % MOD;
        }
    }
}
