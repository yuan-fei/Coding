...
class Solution {
    public int minimumEffort(int[][] tasks) {
    	...
    	Arrays.sort(tasks, (a, b) -> -Integer.compare(a[1] - a[0], b[1] - b[0]));
       	for(int[] task : tasks){
       		int taskMaxRequired = Math.max(task[0], task[1]);
       		if(remaining < taskMaxRequired){
       			minRequired += taskMaxRequired - remaining;
       			// System.out.println(minRequired);
       		}
       		remaining = Math.max(taskMaxRequired, remaining);
       		remaining -= task[0];
       	}
       	return minRequired;
    }
