...
class Solution {
    public int swimInWater(int[][] grid) {
        PriorityQueue<Cell> q = new PriorityQueue<Cell>((a,b)->Integer.compare(a.estimateTime(), b.estimateTime()));
        int N = grid.length;
        boolean[][] visited = new boolean[N][N];
        q.offer(new Cell(0, 0, grid[0][0], grid));
        visited[0][0] = true;
        ...
        while(!q.isEmpty()){
        	...
        	for(int i = 0; i < dir.length; i++){
        		...
        		if(xx>=0 && yy>=0 && xx<N && yy<N && !visited[xx][yy]){
        			q.offer(new Cell(xx, yy, Math.max(cur.t, grid[xx][yy]), grid));
        			visited[xx][yy] = true;
        		}
        	}
        }
    ...
    static class Cell{
    	...
    	int t;
    	int[][] g;
    	Cell(int xx, int yy, int tt, int[][] gg){
    		x=xx;
    		y=yy;
    		t=tt;
    		g=gg;
    	...
    	int estimateTime(){
    		return t + Math.max(0, g[g.length-1][g.length-1]-t);
    	}
    }
}
