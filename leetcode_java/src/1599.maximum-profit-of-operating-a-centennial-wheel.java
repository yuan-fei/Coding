 ...
 * number of new customers arriving just before the i^th rotation (0-indexed).
 ...
 * = $100.
 * 6. 4 board and 1 waits, the wheel rotates. Current profit is 24 * $6 - 6 *
 * $4 = $120.
 ...
 * 6. 4 board and 13 wait, the wheel rotates. Current profit is 24 * $3 - 6 *
 * $8 = $24.
 * 7. 4 board and 9 wait, the wheel rotates. Current profit is 28 * $3 - 7 * $8
 * = $28.
 * 8. 4 board and 5 wait, the wheel rotates. Current profit is 32 * $3 - 8 * $8
 * = $32.
 * 9. 4 board and 1 waits, the wheel rotates. Current profit is 36 * $3 - 9 *
 * $8 = $36.
 * 10. 1 board and 0 wait, the wheel rotates. Current profit is 37 * $3 - 10 *
 * $8 = $31.
...
class Solution {
    public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
        ...
        for(; i < customers.length; i++){
        	cur += customers[i];
        	sum += Math.min(4, cur) * boardingCost;
        	cur = Math.max(cur - 4, 0);
        	if(max < sum - (i + 1) * runningCost){
        		max = sum - (i + 1) * runningCost;
        		minTime = i + 1;
        	}
        }
        while(cur > 0){
