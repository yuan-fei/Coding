...
class Solution {
    public int wiggleMaxLength(int[] nums) {
        ...
        return Math.max(maxWiggleLength(true, nums), maxWiggleLength(false, nums));
    }

    private int maxWiggleLength(boolean headsUp, int[] nums){
    	int target = nums[0];
    	int cnt = 1;
    	for (int n : nums) {
    		if(n != target && headsUp == (Integer.compare(n, target) > 0)){
