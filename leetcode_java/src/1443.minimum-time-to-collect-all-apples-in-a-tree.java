...
class Solution {
    public int minTime(int n, int[][] edges, List<Boolean> hasApple) {
        ...
        return minTime(adj, hasApple, -1, 0);
    }

    int minTime(List<Integer>[] adj, List<Boolean> hasApple, int p, int u){
    	int ret = 0;
    	for(int v : adj[u]){
    		if(v != p){
    			int subTime = minTime(adj, hasApple, u, v);
    			if(subTime > 0){
    				ret += subTime + 2;
    			}
    			else if(hasApple.get(v)){
    				ret += 2;
    			}
    		}
    	}
