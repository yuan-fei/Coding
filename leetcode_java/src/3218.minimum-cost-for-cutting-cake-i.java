...
class Solution {
    ...
    public int minimumCost(int m, int n, int[] horizontalCut, int[] verticalCut) {
        hCost = horizontalCut;
        vCost = verticalCut;
        cache = new int[m][n][m][n];
        return minCost(0, 0, m - 1, n - 1);
    }

    int minCost(int top, int left, int bottom, int right) {
        if (top == bottom && left == right) {
            return 0;
        }
        if (cache[top][left][bottom][right] == 0) {
            int min = Integer.MAX_VALUE;
            for (int hCut = top; hCut < bottom; hCut++) {
                int cost = hCost[hCut] + minCost(top, left, hCut, right) + minCost(hCut + 1, left, bottom, right);
                min = Math.min(cost, min);
            }
            for (int vCut = left; vCut < right; vCut++) {
                int cost = vCost[vCut] + minCost(top, left, bottom, vCut) + minCost(top, vCut + 1, bottom, right);
                min = Math.min(cost, min);
            }
            cache[top][left][bottom][right] = min;
        }

        return cache[top][left][bottom][right];
    }
}
// @lc code=end
