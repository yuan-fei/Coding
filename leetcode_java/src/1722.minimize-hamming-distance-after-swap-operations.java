...
class Solution {
    public int minimumHammingDistance(int[] source, int[] target, int[][] allowedSwaps) {
    	...
    	for(int i = 0; i < n; i++){
    		Set<Integer> s = m.getOrDefault(source[i], new HashSet<>());
    		s.add(i);
    		m.put(source[i], s);
    	}
    	Map<Integer, List<Integer>> components = dsu.getComponents();
    	int diff = 0;
    	for(List<Integer> l : components.values()){
    		diff += getDiff(l, source, target);
    	}
    	return diff;
    }
    ...
    int getDiff(List<Integer> l, int[] source, int[] target){
    ...
    class DSU {
		...
		Map<Integer, List<Integer>> getComponents(){
			Map<Integer, List<Integer>> m = new HashMap<>();
			for (int i = 0; i < parent.length; i++) {
				int p = find(i);
				List<Integer> s = m.getOrDefault(p, new ArrayList<>());
	    		s.add(i);
	    		m.put(p, s);
			}
			return m;
