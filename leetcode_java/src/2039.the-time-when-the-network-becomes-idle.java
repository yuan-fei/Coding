 ...
 *
 * There is a network of n servers, labeled from 0 to n - 1. You are given a 2D
 ...
 * 
 * At second 1,
 * - Message 1A arrives at the master server. Master server processes message
 * 1A instantly and sends a reply 1A back.
 * - Server 1 has not received any reply. 1 second (1 < patience[1] = 2)
 * elapsed since this server has sent the message, therefore it does not resend
 * the message.
 ...
 * 1.
 * - Message 2A arrives at the master server. Master server processes message
 * 2A instantly and sends a reply 2A back.
...
class Solution {
    ...
    int bfs(int[] patience){
        ...
        while(!q.isEmpty()){
            for(int i = q.size(); i > 0; i--){
                int cur = q.poll();
                max = Math.max(max, calcTime(t, patience[cur]));
                for(int nxt : adj[cur]){
                    if(!visited[nxt]){
                        q.offer(nxt);
                        visited[nxt] = true;
    ...
    int calcTime(int t, int patience){
        // System.out.println(t + ", " + patience);
        if(patience == 0){
            return 0;
        }
        return t * 2 + (t * 2 - 1) / patience * patience;
    }
}
// @lc code=end
