...
class Solution {
    public int[] smallestTrimmedNumbers(String[] nums, int[][] queries) {
        ...
        List<Integer>[] indexedQueries = new List[101];
        int[] idx = new int[m];
        for(int i = 0; i <= 100; i++){
            indexedQueries[i] = new ArrayList<Integer>();
        }
        for(int i = 0; i < m; i++){
            idx[i] = i;
        }
        for(int i = 0; i < n; i++){
            indexedQueries[queries[i][1] - 1].add(i);
        }
        int[] ans = new int[n];
        for(int i = 0; i < nums[0].length(); i++){
            radixSort(nums, idx, nums[0].length() - i - 1);
            // System.out.println(indexedQueries[i]);
            for(int j : indexedQueries[i]){
                ans[j] = idx[queries[j][0] - 1];
            }
        }
        return ans;
    }

    void radixSort(String[] a, int[] idx, int pos){
        List<Integer>[] bucket = new List[10];
        for(int i = 0; i < 10; i++){
            bucket[i] = new ArrayList<>();
        }
        for(int i : idx){
            bucket[a[i].charAt(pos) - '0'].add(i);
        }
        int i = 0;
