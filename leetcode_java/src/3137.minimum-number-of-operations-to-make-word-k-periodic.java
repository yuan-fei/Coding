...
class Solution {
    public int minimumOperationsToMakeKPeriodic(String word, int k) {
        Map<String, Integer> freq = IntStream.range(0, word.length() / k)
                .mapToObj(start -> word.substring(start * k, start * k + k))
                .collect(Collectors.toMap(s -> s, _ -> 1, (v1, v2) -> v1 + v2));
        // System.out.println(freq);
        return word.length() / k - freq.values().stream().mapToInt(n -> n).max().getAsInt();
    }
}
// @lc code=end
