...
class Solution {
    ...
    void dfs(int cookiesCur, int distCur){
        if(cookiesCur == cookies.length){
            int max = 0;
            for(int x : dist){
                max = Math.max(max, x);
            }
        ...
        for(int i = cookiesCur; i < cookies.length; i++){
            swap(cookies, cookiesCur, i);
            dist[distCur] += cookies[cookiesCur];
            dfs(cookiesCur + 1, distCur);
            dist[distCur] -= cookies[cookiesCur];
            if(distCur + 1 < k){
                dist[distCur + 1] += cookies[cookiesCur];
                dfs(cookiesCur + 1, distCur + 1);
                dist[distCur + 1] -= cookies[cookiesCur];
            }
            swap(cookies, cookiesCur, i);
        }
