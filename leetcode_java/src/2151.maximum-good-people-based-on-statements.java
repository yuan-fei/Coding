 ...
 * they could be:
 * ⁠       - telling the truth. Following this scenario, person 0 and 1 are
 * both bad as explained before.
 ...
 * person.
 * ⁠           - Following that person 2 is bad and lied, there will be two
 ...
 * be no good persons in the group.
 * ⁠       - lying. In this case person 1 is a good person.
 * ⁠           - Following that person 0 is bad and lied, there will be only
 * one good person in the group.
 * We can see that at most, one person is good in the best case, so we return
 * 1.
 * Note that there is more than one way to arrive at this conclusion.
 * 
 * 
...
class Solution {
    public int maximumGood(int[][] statements) {
        ...
        int max = 0;
        for(int m = 0; m < (1 << n); m++){
            if(verify(statements, m)){
                max = Math.max(max, Integer.bitCount(m));
            }
        }
        return max;
    }

    boolean verify(int[][] statements, int m){
        int n = statements.length;
        for(int i = 0; i < n; i++){
            int p1 = (m >> i) & 1;
            for(int j = 0; j < n; j++){
                if(i != j){
                    int p2 = (m >> j) & 1;
                    if(p1 == 1 && statements[i][j] != 2 && statements[i][j] != p2){
                        // System.out.println(p1 + ", " + p2 + "=" + i + ", " + j);
                        return false;
                    }    
                }
            }
