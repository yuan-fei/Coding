...
public class Solution {
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
		...
		return getIntersectionNode(headA, headB, la, lb);
    }

    ListNode getIntersectionNode(ListNode headA, ListNode headB, int la, int lb){
    	// System.out.println(la + ", " + lb);
    	if(la > lb){
    		return getIntersectionNode(headB, headA, lb, la);
    	}
    	else if(la < lb){
    		return getIntersectionNode(headA, headB.next, la, lb - 1);
    	}
    	else{
    		if(headA == headB){
    			return headB;
    			...
    			return getIntersectionNode(headA.next, headB.next, la - 1, lb - 1);
    		}
    	}
    }
