 ...
 * represents a price if it is a non-negative real number preceded by a dollar
 * sign.
 * 
 * 
 ...
 * 
 * 
 * Input: sentence = "there are $1 $2 and 5$ candies in the shop", discount =
 * 50
 * Output: "there are $0.50 $1.00 and 5$ candies in the shop"
 * Explanation: 
 * The words which represent prices are "$1" and "$2". 
 * - A 50% discount on "$1" yields "$0.50", so "$1" is replaced by "$0.50".
 * - A 50% discount on "$2" yields "$1". Since we need to have exactly 2
 * decimal places after a price, we replace "$2" with "$1.00".
 * 
 * 
 * Example 2:
 * 
    ...
    public String discountPrices(String sentence, int discount) {
        ...
        for(int i = 0; i < parts.length; i++){
            if(parts[i].matches("\\$\\d+")){
                double d = Double.parseDouble(parts[i].substring(1));
                d = d * (100 - discount) / 100;
                parts[i] = String.format("$%.2f", d);
            }
        }
