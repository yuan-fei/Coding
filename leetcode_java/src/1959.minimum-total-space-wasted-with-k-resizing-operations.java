 ...
 * at time 3.
 * The total wasted space is (10 - 10) + (20 - 20) + (20 - 15) + (30 - 30) +
 * (30 - 20) = 15.
 * 
 * 
 * 
...
class Solution {
    public int minSpaceWastedKResizing(int[] nums, int k) {
        ...
        for(int i = 1; i <= n; i++){
            ...
            maxGlobal = Math.max(maxGlobal, nums[i - 1]);
            dp[i][0] = maxGlobal * i - pSum[i];
        }
        
        
        for(int i = 1; i <= n; i++){
            Arrays.fill(dp[i], 1, k + 1, MAX);
            for(int j = 1; j <= k; j++){
                int maxValue = 0;
                for(int prv = i; prv >= 1; prv--){
                    maxValue = Math.max(maxValue, nums[prv - 1]);
                    // System.out.println(Arrays.asList(i, j, prv, dp[i][j], dp[prv - 1][j - 1] + maxValue * (i - prv + 1) - (pSum[i] - pSum[prv - 1]), dp[prv - 1][j - 1], maxValue * (i - prv + 1) - (pSum[i] - pSum[prv - 1])));
                    dp[i][j] = Math.min(dp[i][j], dp[prv - 1][j - 1] + maxValue * (i - prv + 1) - (pSum[i] - pSum[prv - 1]));
                }
            }
        }
