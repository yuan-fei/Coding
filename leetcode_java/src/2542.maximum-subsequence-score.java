...
class Solution {
    public long maxScore(int[] nums1, int[] nums2, int k) {
        TreeMap<Integer, List<Integer>> m = new TreeMap<>(Comparator.comparing(x -> -x));
        for(int i = 0; i < nums2.length; i++){
            m.putIfAbsent(nums2[i], new ArrayList<Integer>());
            m.get(nums2[i]).add(nums1[i]);
        }
        ...
        for(int key : m.keySet()){
            Collections.sort(m.get(key), Comparator.comparing(x -> -x));
            for(int v : m.get(key)){
                sum += v;
                if(pq.size() == k){
                    sum -= pq.poll();
                ...
                if(pq.size() == k){
                    score = sum * key;
                    // System.out.println(Arrays.asList(key, v, sum));
