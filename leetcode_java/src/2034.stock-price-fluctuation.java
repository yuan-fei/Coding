...
class StockPrice {
    ...
    TreeMap<Integer, Integer> ts2Price = new TreeMap<>();
    TreeMap<Integer, Integer> price2Cnt = new TreeMap<>();
    public StockPrice() {
        
    }
    
    public void update(int timestamp, int price) {
        Integer oldValue = ts2Price.put(timestamp, price);
        if(oldValue != null){
            int cnt = price2Cnt.get(oldValue) - 1;
            if(cnt == 0){
                price2Cnt.remove(oldValue);
            }
            else{
                price2Cnt.put(oldValue, cnt);
            }
        }
        price2Cnt.put(price, price2Cnt.getOrDefault(price, 0) + 1);
    }
    
    public int current() {
        return ts2Price.lastEntry().getValue();
    }
