...
class Skiplist {
    ...
    class Node {
        ...
        int getHighestLayer(){
            return nextByLayers.size();
        }

        void increaseLayer(){
            nextByLayers.add(null);
        }
    }
    ...
    public boolean search(int target) {
        Node prevNode = head;
        for(int l = head.getHighestLayer() - 1; l >= 0; l--){
            while(prevNode.nextByLayers.get(l) != null && prevNode.nextByLayers.get(l).val < target){
                prevNode = prevNode.nextByLayers.get(l);
            }
            if(prevNode.nextByLayers.get(l) != null && prevNode.nextByLayers.get(l).val == target){
                return true;
            }
        }
        return false;
    ...
    public void add(int num) {
        int maxLayer = getRandomHighestLayer();
        while(maxLayer > head.getHighestLayer()){
            increaseLayer();
        }
        Node target = new Node(num, maxLayer);
        Node prevNode = head;
        for(int l = head.getHighestLayer() - 1; l >= 0; l--){
            while(prevNode.nextByLayers.get(l) != null && prevNode.nextByLayers.get(l).val <= num){
                prevNode = prevNode.nextByLayers.get(l);
            }
            if(l < maxLayer){
                target.nextByLayers.set(l, prevNode.nextByLayers.get(l));
                prevNode.nextByLayers.set(l, target);
            }
        }
    }
    ...
    public boolean erase(int num) {
        ...
        for(int l = head.getHighestLayer() - 1; l >= 0; l--){
            ...
            if(prevNode.nextByLayers.get(l) != null && prevNode.nextByLayers.get(l).val == num){
                prevNode.nextByLayers.set(l, prevNode.nextByLayers.get(l).nextByLayers.get(l));
                removed = true;
            }
        }
        return removed;
    ...
    private int getRandomHighestLayer(){
        int ret = 1;
