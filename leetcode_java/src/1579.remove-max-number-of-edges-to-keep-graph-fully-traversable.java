...
class Solution {
    ...
    public int maxNumEdgesToRemove(int n, int[][] edges) {
    	...
    	if(!checkTraversable(new HashSet<>(Arrays.asList(1, 3))) || !checkTraversable(new HashSet<>(Arrays.asList(2, 3)))){
    		return -1;
    	}
		...
		for(int i = 1; i < visited.length; i++){
			if(!visited[i]){
				holes++;
				dfs(new HashSet<>(Arrays.asList(3)), i, visited);
			}
		}
		holes--;
		System.out.println(holes);
		int minEdges = (n - 1) - holes + 2 * holes;
		return cnt[1] + cnt[2] + cnt[3] - minEdges;
    }

    boolean checkTraversable(Set<Integer> legalTypes){
    	boolean[] visited = new boolean[n + 1];
		dfs(legalTypes, 1, visited);
		for(int i = 1; i < visited.length; i++){
			if(!visited[i]){
				return false;
			}
    ...
    void dfs(Set<Integer> legalTypes, int u, boolean[] visited){
    	visited[u] = true;
    	for(int e: adj[u]){
    		int v = edges[e][2] + edges[e][1] - u; 
    		if(legalTypes.contains(edges[e][0]) && !visited[v]){
    			dfs(legalTypes, v, visited);
    		}
    	}
    }
}
