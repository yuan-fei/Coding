 ...
 * 
 * The time taken by the first processor to finish the execution of all tasks
 * is max(8 + 8, 8 + 7, 8 + 4, 8 + 5) = 16.
 * 
 * The time taken by the second processor to finish the execution of all tasks
 * is max(10 + 2, 10 + 2, 10 + 3, 10 + 1) = 13.
 * 
 * 
 * Example 2:
 * 
 ...
 * 
 * The time taken by the first processor to finish the execution of all tasks
 * is max(10 + 3, 10 + 5, 10 + 8, 10 + 4) = 18.
 * 
 * The time taken by the second processor to finish the execution of all tasks
 * is max(20 + 2, 20 + 1, 20 + 2, 20 + 3) = 23.
 * 
 * 
 * 
 * Constraints:
...
class Solution {
    public int minProcessingTime(List<Integer> processorTime, List<Integer> tasks) {
        ...
        List<Integer> maxInGroup = new ArrayList<>(Collections.nCopies(processorTime.size(), 0));
        for(int i = 0; i < tasks.size(); i++){
            maxInGroup.set(i / 4, Math.max(tasks.get(i), tasks.get(i / 4)));
        }
        Collections.reverse(maxInGroup);
        int ans = 0;
        for(int i = 0; i < processorTime.size(); i++){
            ans = Math.max(processorTime.get(i) + maxInGroup.get(i), ans);
        }
        return ans;
    }
}
