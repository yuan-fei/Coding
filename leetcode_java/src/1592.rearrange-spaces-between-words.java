...
class Solution {
    public String reorderSpaces(String text) {
        ...
        if(words.length == 1){
        	String extra = getSpace(text.length() - words[0].length());	
        	return words[0] + extra;
        }
        int cnt = total / (words.length - 1);
        int extraCnt = total - cnt * (words.length - 1);
        String delimiter = getSpace(cnt);
        String extra = getSpace(extraCnt);
        String ans = String.join(delimiter, words);
        ans += extra;
        return ans;
    }
