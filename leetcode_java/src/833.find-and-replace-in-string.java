...
class Solution {
    public String findReplaceString(String S, int[] indexes, String[] sources, String[] targets) {
        ...
        for(int i = indexes.length - 1; i >= 0 ; i--){
            String s = sources[map.get(indexes[i])];
            String t = targets[map.get(indexes[i])];
            if(indexes[i] + s.length() <= end){
                boolean replace = true;
                for(int j = 0; j < s.length(); j++){
                    if(ret.charAt(j + indexes[i]) != s.charAt(j)){
                        replace = false;
                        break;
                    }
                }
                if(replace){
                    ret = ret.substring(0, indexes[i]) + t + ret.substring(indexes[i] + s.length());
                }
            }
            end = indexes[i];
        }
