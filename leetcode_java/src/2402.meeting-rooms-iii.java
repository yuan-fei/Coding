...
class Solution {
    public int mostBooked(int n, int[][] meetings) {
        ...
        TreeMap<Long, TreeSet<Integer>> tm = new TreeMap<>();
        tm.put(0L, new TreeSet<Integer>());
        for(int i = 0; i < n; i++){
            tm.get(0L).add(i);
        }
        ...
        for (int[] m : meetings) {
            ...
            while(!tm.isEmpty() && tm.firstKey() <= m[0]){
                ts.addAll(tm.pollFirstEntry().getValue());
            }
            if(!ts.isEmpty()){
                tm.put((long)m[0], ts);
            }
            long lastEnd = tm.firstKey();
            int r = tm.get(lastEnd).pollFirst();
            cnt[r]++;
            if(tm.get(lastEnd).isEmpty()){
                tm.remove(lastEnd);
            }
            long nextEnd = Math.max(lastEnd, m[0]) + (m[1] - m[0]);
            if(!tm.containsKey(nextEnd)){
                tm.put(nextEnd, new TreeSet<>());
            }
            tm.get(nextEnd).add(r);
